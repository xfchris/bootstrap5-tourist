...
(function(window, factory) {
    ...
    Tour = (function() {
        ...
        Tour.prototype.end = function() {
            ...
            promise = this.hideStep();
            $('.popover-tour.fade.show').fadeOut(120, function() {
                $('.popover-tour.fade.show').popover('dispose');
            });
            ...
            if ($(document).find(".popover.tour-" + this._options.name + ".tour-" + this._options.name + "-" + this.getCurrentStepIndex()).length == 0) {
                ...
                if (step.showProgressBar) {
                    if (typeof(step.getProgressBarHTML) == "function") {
                        ...
                        content = '<div class="progress"><div class="progress-bar progress-bar-striped" role="progressbar" style="width: ' + percentProgress + '%;" data-percent="' + percentProgress + '%"></div></div>' + content;
                    }
                }
                ...
                if (this._options.framework == "bootstrap5") {
                    popOpts['allowList'] = this._options.sanitizeWhitelist;
                } else {
                    popOpts['whiteList'] = this._options.sanitizeWhitelist;
                }

                if (this._options.framework == "bootstrap4" || this._options.framework == "bootstrap5") {
                    if (isOrphan && this._options.framework == "bootstrap4") {
                    ...
                    } else if (isOrphan && this._options.framework == "bootstrap5") {
                        popOpts.offset = function(obj) {
                            var top = Math.max(0, (($(window).height() - obj.popper.height) / 2) + obj.popper.height);
                            return [0, top * -1];
                        };
                    } else {
                ...
                if (this._options.framework == "bootstrap4" || this._options.framework == "bootstrap5") {
                    $tipElement = $element.data('bs.popover') ? $element.data('bs.popover').getTipElement() : $('#' + $element.attr('aria-describedby'))[0];
                    $tip = $($tipElement);
                }
